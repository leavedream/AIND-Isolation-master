...
def custom_score(game, player):
    ...
    # heurisitic function 1: use #my_move - 2*#oppo_move in the beginning of same, 
    # use #my_move - #oppo_move in the end of the game
    # return heuristic_space(game, player)
    
    # heurisitic function 2: use #my_move / #oppo_move
    return heuristic_move_divide(game, player)
    
    # heurisitic function 3: use ln(#my_move) / ln(#oppo_move)
    # return heuristic_move_exp(game, player)

def heuristic_move_exp(game, player):
    """Calculate the heuristic value of a game state from the point of view
    ...
    score_opp = len(game.get_legal_moves(game.get_opponent(player)))
    if score_opp == 0:
        return float("inf")
        
    return math.exp(score) / math.exp(score_opp)
...
def heuristic_space(game, player):
    ...
    if blanks > area // 2:
        return float(my_move - 2 * oppo_move)
        
    return float(my_move - oppo_move)
    ...
    def get_move(self, game, legal_moves, time_left):
        ...
        **********************************************************************
        ...
        try:
            ...
            if(len(moves) == 0):
                return (-1, -1)
            # Initialize move to the first abailable move in case exception occurs    
            move = moves[0]     
